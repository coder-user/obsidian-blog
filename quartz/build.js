...
async function buildQuartz(argv, mut, clientRefresh) {
    ...
    const filePaths = fps.map((fp) => joinSegments(argv.directory, fp));
...
async function startServing(ctx, mut, initialContent, clientRefresh) {
    ...
    async function rebuild(fp, action) {
        ...
        fp = toPosixPath(fp);
        const filePath = joinSegments(argv.directory, fp);
        ...
        try {
            ...
            const trackedSlugs = [...new Set([...contentMap.keys(), ...toRebuild, ...trackedAssets])]
                ...
                .map((fp) => slugifyFilePath(path.posix.relative(argv.directory, fp)));
