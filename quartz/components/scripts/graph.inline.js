...
async function renderGraph(container, fullSlug) {
    ...
    let { drag: enableDrag, zoom: enableZoom, depth, scale, repelForce, centerForce, linkDistance, fontSize, opacityScale, removeTags, showTags, } = JSON.parse(graph.dataset["cfg"]);
    const data = await fetchData;
    const links = [];
    const tags = [];
    const validLinks = new Set(Object.keys(data).map((slug) => simplifySlug(slug)));
    ...
    if (depth >= 0) {
        ...
        Object.keys(data).forEach((id) => neighbourhood.add(simplifySlug(id)));
        if (showTags)
