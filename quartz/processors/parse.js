...
export function createProcessor(ctx) {
    ...
    return async (processor) => {
        ...
        for (const fp of fps) {
            try {
                ...
                file.data.slug = slugifyFilePath(path.posix.relative(argv.directory, file.path));
...
export async function parseMarkdown(ctx, fps) {
    ...
    if (concurrency === 1) {
        ...
        const results = await WorkerPromise.all(childPromises).catch((err) => {
