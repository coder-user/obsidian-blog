...
describe("transforms", () => {
    function asserts(pairs, transform, checkPre, checkPost) {
    ...
    test("simplifySlug", () => {
        asserts([
            ...
            ["abc/index", "abc/"],
            ["abc/def", "abc/def"],
        ], path.simplifySlug, path.isFullSlug, path.isSimpleSlug);
    });
    test("slugifyFilePath", () => {
        ...
        ], path.slugifyFilePath, path.isFilePath, path.isFullSlug);
    });
    test("transformInternalLink", () => {
        ...
        ], path.transformInternalLink, (_x) => true, path.isRelativeURL);
    });
    test("pathToRoot", () => {
        asserts([
            ["index", "."],
            ...
            ["abc/def/ghi", "../.."],
            ["abc/def/index", "../.."],
        ], path.pathToRoot, path.isFullSlug, path.isRelativeURL);
    });
});
